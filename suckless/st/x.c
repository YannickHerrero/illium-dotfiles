...
xloadcols(void)
{
	...
	dc.col[defaultbg].pixel &= 0x00FFFFFFu;
...
xinit(int cols, int rows)
{
	...
	/* use TrueColor visual for alpha support */
	if (!(XMatchVisualInfo(xw.dpy, xw.scr, 32, TrueColor, vis))) {
		XVisualInfo vi;
		vi.visual = XDefaultVisual(xw.dpy, xw.scr);
		vi.depth = DefaultDepth(xw.dpy, xw.scr);
		vi.screen = xw.scr;
		vis = &vi;
	}
	xw.vis = vis->visual;
	...
	xw.win = XCreateWindow(xw.dpy, parent, xw.l, xw.t,
			win.w, win.h, 0, vis->depth, InputOutput,
