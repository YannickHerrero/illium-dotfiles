...
static void tagandfollow(const Arg *arg);
...
movestack(const Arg *arg)
{
	...
	if (c && c != selmon->sel) {
		Client *temp = selmon->sel->next;
		selmon->sel->next = c->next;
		c->next = (temp == c) ? selmon->sel : temp;
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + gappx, m->wy + my + gappx, mw - (2*c->bw), h - (2*c->bw), 0);
			if (my + HEIGHT(c) + gappx < m->wh)
				my += HEIGHT(c) + gappx;
		} else {
			h = (m->wh - ty - 2 * gappx) / (n - i);
			resize(c, m->wx + mw + gappx, m->wy + ty + gappx, m->ww - mw - 2 * gappx - (2*c->bw), h - (2*c->bw), 0);
