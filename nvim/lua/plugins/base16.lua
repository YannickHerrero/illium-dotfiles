-- Pywal16-driven colorscheme via nvim-base16
-- Colors are auto-generated by wal-set.sh into lua/theme-colors.lua
return {
  {
    "RRethy/nvim-base16",
    lazy = false,
    ...
    config = function()
      ...
      -- Live reload: when switching back to neovim, check if
      -- theme-colors.lua was regenerated by wal-set.sh and reload
      vim.api.nvim_create_autocmd("FocusGained", {
        group = "Base16CustomHighlights",
        pattern = "*",
        callback = function()
          -- Clear cached module so require() re-reads from disk
          package.loaded["theme-colors"] = nil
          local rok, new_colors = pcall(require, "theme-colors")
          if not rok or not new_colors.base16 then
            return
          end
          -- Only reload if colors actually changed
          local cur = require("base16-colorscheme").colors
          if cur and cur.base00 == new_colors.base16.base00
            and cur.base05 == new_colors.base16.base05
            and cur.base0D == new_colors.base16.base0D then
            return
          end
          require("base16-colorscheme").setup(new_colors.base16)
        end,
      })
    end,
  },
